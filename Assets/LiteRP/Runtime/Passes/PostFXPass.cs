...
using static LiteRP.Runtime.PostFXStack;
...
namespace LiteRP.Runtime
{
    public class PostFXPass
    {
        private static readonly ProfilingSampler
            groupSampler = new("Render PostProcess Pass"),
            finalSampler = new("Final PostProcess Pass");
        
        private static readonly int
            copyBicubicId = Shader.PropertyToID("_CopyBicubic"),
            fxaaConfigId = Shader.PropertyToID("_FXAAConfig");

        private static readonly GlobalKeyword
		...
		public static void Record(RenderGraph renderGraph, PostFXStack stack, int colorLUTResolution,
			bool keepAlpha, in CameraRendererTextures textures)
		{
			...
			using RenderGraphBuilder builder =
				renderGraph.AddRenderPass(finalSampler.name, out PostFXPass pass, finalSampler);
			...
			{
				pass.scaleMode =
					stack.BufferSettings.bicubicRescaling == CameraBufferSettings.BicubicRescalingMode.UpAndDown ||
					stack.BufferSettings.bicubicRescaling == CameraBufferSettings.BicubicRescalingMode.UpOnly &&
					stack.BufferSize.x < stack.Camera.pixelWidth ? ScaleMode.Bicubic : ScaleMode.Linear;
			}
