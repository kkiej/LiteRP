...
float4 GetMask(InputConfig c)
{
    if(c.useMask)
    {
        return SAMPLE_TEXTURE2D(_MaskMap, sampler_BaseMap, c.UV);
    }
    return 1.0;
}
...
{
    float4 map = SAMPLE_TEXTURE2D(_BaseMap, sampler_BaseMap, c.UV);
...
{
    float4 map = SAMPLE_TEXTURE2D(_NormalMap, sampler_BaseMap, c.UV);
