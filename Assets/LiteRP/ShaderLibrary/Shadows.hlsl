#ifndef LITERP_SHADOWS_INCLUDED
#define LITERP_SHADOWS_INCLUDED
...
ShadowData GetShadowData(Surface surfaceWS)
{
	...
	for(i = 0; i < _CascadeCount; i++)
	{
		...
		{
			float fade = FadedShadowStrength(distanceSqr, cascade.data.x, _ShadowDistanceFade.z);
...
{
	float3 normalBias = surfaceWS.interpolatedNormal * (directional.normalBias * _DirectionalShadowCascades[global.cascadeIndex].data.y);
	...
	{
		normalBias = surfaceWS.interpolatedNormal * (directional.normalBias * _DirectionalShadowCascades[global.cascadeIndex + 1].data.y);
...
static const float3 pointShadowPlanes[6] = { float3(-1.0, 0.0, 0.0),
											 float3(1.0, 0.0, 0.0),
											 float3(0.0, -1.0, 0.0),
											 float3(0.0, 1.0, 0.0),
											 float3(0.0, 0.0, -1.0),
...
float GetOtherShadow(OtherShadowData other, ShadowData global, Surface surfaceWS)
{
	...
	float4 positionSTS = mul(data.shadowMatrix[tileIndex], float4(surfaceWS.position + normalBias, 1.0));
