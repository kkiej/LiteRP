...
Light GetDirectionalLight( int index, Surface surfaceWS, ShadowData shadowData)
{
	...
	//light.renderingLayerMask = asuint(data.directionAndMask.w); 使用asuint会失效
	light.renderingLayerMask = data.directionAndMask.w;
...
Light GetOtherLight(int index, Surface surfaceWS, ShadowData shadowData)
{
	...
	float spotAttenuation = Square(saturate(dot(spotDirection,
		light.direction) * data.spotAngle.x + data.spotAngle.y));
